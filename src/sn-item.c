...
static void sn_item_get_property (GObject * object, guint prop_id,
                                  GValue * value, GParamSpec * pspec);

static void sn_item_set_property (GObject * object, guint prop_id,
...
sn_item_class_init (SnItemClass * klass)
{
  ...
  g_object_class_install_property (object_class,
                                   PROP_BUS_NAME,
                                   g_param_spec_string ("bus-name", NULL,
                                                        NULL, NULL,
                                                        G_PARAM_WRITABLE |
  ...
  g_object_class_install_property (object_class,
                                   PROP_OBJECT_PATH,
                                   g_param_spec_string ("object-path", NULL,
                                                        NULL, NULL,
                                                        G_PARAM_WRITABLE |
                                                        G_PARAM_STATIC_STRINGS));
  ...
  g_object_class_install_property (object_class,
                                   PROP_EXPOSED,
                                   g_param_spec_boolean ("exposed", NULL,
                                                         NULL, FALSE,
                                                         G_PARAM_READABLE |
                                                         G_PARAM_STATIC_STRINGS));
...
static void
sn_item_get_property (GObject * object, guint prop_id, GValue * value,
                      GParamSpec * pspec)
{
  SnItem *item = SN_ITEM (object);
...
static void
sn_item_set_property (GObject * object, guint prop_id, const GValue * value,
                      GParamSpec * pspec)
{
  SnItem *item = SN_ITEM (object);
...
static void
sn_item_properties_callback (GObject * source_object, GAsyncResult * res,
                             gpointer user_data)
{
  SnItem *item = user_data;
  GError *error = NULL;
...
static void
sn_item_item_callback (GObject * source_object, GAsyncResult * res,
                       gpointer user_data)
{
  SnItem *item = user_data;
  GError *error = NULL;
  ...
  context->handler =
    g_dbus_connection_signal_subscribe (g_dbus_proxy_get_connection
                                        (item->item_proxy),
                                        "org.freedesktop.DBus",
                                        "org.freedesktop.DBus",
                                        "NameOwnerChanged",
                                        "/org/freedesktop/DBus",
                                        g_dbus_proxy_get_name (item->
                                                               item_proxy),
                                        G_DBUS_SIGNAL_FLAGS_NONE,
                                        sn_item_name_owner_changed, item,
                                        NULL);
  ...
  g_dbus_proxy_new (g_dbus_proxy_get_connection (item->item_proxy),
                    ...
                    item->bus_name,
                    item->object_path,
                    "org.freedesktop.DBus.Properties", item->cancellable,
                    sn_item_properties_callback, item);
}
...
{
  g_return_if_fail (IS_SN_ITEM (item));
  ...
  g_dbus_proxy_new_for_bus (G_BUS_TYPE_SESSION,
                            ...
                            item->bus_name,
                            item->object_path,
                            "org.kde.StatusNotifierItem", item->cancellable,
                            sn_item_item_callback, item);
}
...
sn_item_invalidate (SnItem * item)
{
  ...
  g_dbus_proxy_call (item->properties_proxy,
                     ...
                     G_DBUS_CALL_FLAGS_NONE, -1, item->cancellable,
                     sn_item_get_all_properties_result, item);
}
      ...
      if (exposed != item->exposed)
        {
          ...
          if (item->initialized)
            g_signal_emit (G_OBJECT (item),
                           sn_item_signals[exposed ? EXPOSE : SEAL], 0);
        }
    }
}

static gboolean
sn_item_pixbuf_equals (GdkPixbuf * p1, GdkPixbuf * p2)
{
  guchar *p1p, *p2p;
  guint p1l, p2l;
  guint i;
...
static void
sn_item_free (guchar * pixels, gpointer data)
{
  g_free (pixels);
}
      ...
      if (!g_strcmp0 (name, "Id"))
      ...
      else if (!g_strcmp0 (name, "ToolTip"))
        {
          ...
          if (!g_strcmp0 (cstr_val1, "(sa(iiay)ss)"))
            {
              g_variant_get (value, "(sa(iiay)ss)", NULL, NULL, &str_val1,
                             &str_val2);
...
#undef string_empty_null
  ...
  if (!item->initialized)
  ...
  else
    {
      if (update_exposed)
        g_signal_emit (G_OBJECT (item),
                       sn_item_signals[item->exposed ? EXPOSE : SEAL], 0);

      if (item->exposed)
        {
          if (update_tooltip)
            g_signal_emit (G_OBJECT (item), sn_item_signals[TOOLTIP_CHANGED],
                           0);
          if (update_icon)
          ...
          if (update_menu)
            {
              ...
              item->cached_menu = NULL;
              g_signal_emit (G_OBJECT (item), sn_item_signals[MENU_CHANGED],
                             0);
            }
        }
    }
...
{
  g_return_val_if_fail (IS_SN_ITEM (item), NULL);
...
sn_item_get_icon (SnItem * item,
                  ...
                  GdkPixbuf ** icon_pixbuf, const gchar ** overlay_icon_name,
                  GdkPixbuf ** overlay_icon_pixbuf)
{
  g_return_if_fail (IS_SN_ITEM (item));
  ...
  if (icon_name != NULL)
    {
      *icon_name =
        ...
        NULL ? item->attention_icon_name : item->icon_name;
    }
  ...
  if (icon_pixbuf != NULL)
    {
      *icon_pixbuf =
        ...
        NULL ? item->attention_icon_pixbuf : item->icon_pixbuf;
    }
...
void
sn_item_get_tooltip (SnItem * item, const gchar ** title,
...
{
  g_return_val_if_fail (IS_SN_ITEM (item), FALSE);
...
void
sn_item_activate (SnItem * item, gint x_root, gint y_root)
{
  g_return_if_fail (IS_SN_ITEM (item));
  g_return_if_fail (item->initialized);
...
void
sn_item_secondary_activate (SnItem * item, gint x_root, gint y_root)
{
  g_return_if_fail (IS_SN_ITEM (item));
  g_return_if_fail (item->initialized);
  g_return_if_fail (item->item_proxy != NULL);
...
void
sn_item_scroll (SnItem * item, gint delta_x, gint delta_y)
{
  g_return_if_fail (IS_SN_ITEM (item));
  g_return_if_fail (item->initialized);
  g_return_if_fail (item->item_proxy != NULL);
