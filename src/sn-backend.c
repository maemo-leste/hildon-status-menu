...
static gboolean sn_backend_watcher_register_item (SnWatcher *
                                                  watcher_skeleton,
...
static gboolean sn_backend_watcher_register_host (SnWatcher *
                                                  watcher_skeleton,
                                                  GDBusMethodInvocation *
                                                  invocation,
                                                  const gchar * service);
...
static void sn_backend_host_remove_item (SnBackend * backend, SnItem * item,
...
sn_backend_new (void)
{
  return g_object_new (TYPE_SN_BACKEND, NULL);
}

void
sn_backend_start (SnBackend * backend)
{
  g_return_if_fail (IS_SN_BACKEND (backend));
  ...
  backend->watcher_bus_owner_id =
    g_bus_own_name (G_BUS_TYPE_SESSION,
                    ...
                    sn_backend_watcher_bus_acquired, NULL,
                    sn_backend_watcher_name_lost, backend, NULL);
  ...
  backend->host_bus_watcher_id =
    g_bus_watch_name (G_BUS_TYPE_SESSION,
                      ...
                      sn_backend_host_name_vanished, backend, NULL);
}
                                 ...
                                 const gchar * name, gpointer user_data)
{
  ...
  sn_watcher_set_is_status_notifier_host_registered (backend->
                                                     watcher_skeleton, TRUE);
  ...
  g_signal_connect (backend->watcher_skeleton,
  ...
  g_dbus_interface_skeleton_export (G_DBUS_INTERFACE_SKELETON
                                    (backend->watcher_skeleton), connection,
                              ...
                              const gchar * name, gpointer user_data)
{
  ...
  if (backend->watcher_skeleton != NULL)
    sn_watcher_set_registered_status_notifier_items (backend->
                                                     watcher_skeleton, NULL);
}
  ...
  if (new_owner == NULL || strlen (new_owner) == 0)
    {
      ...
      g_dbus_connection_signal_unsubscribe (context->connection,
                                            context->handler);
      ...
      sn_watcher_emit_status_notifier_item_unregistered (backend->
                                                         watcher_skeleton,
                                                         key);
      g_free (key);
    }
...
sn_backend_watcher_register_item (SnWatcher * watcher_skeleton,
                                  ...
                                  const gchar * service, SnBackend * backend)
{
  const gchar *bus_name;
  const gchar *object_path;
  const gchar *sender;
  ...
  sn_watcher_complete_register_status_notifier_item (watcher_skeleton,
                                                     invocation);
...
sn_backend_watcher_register_host (SnWatcher * watcher_skeleton,
                                  ...
                                  const gchar * service)
{
  sn_watcher_complete_register_status_notifier_host (watcher_skeleton,
                                                     invocation);
...
static void
sn_backend_watcher_collect_item_keys (gpointer key, gpointer value,
                                      gpointer user_data)
{
  ...
  if (backend->watcher_skeleton != NULL)
    {
      context.index = 0;
      context.out =
        g_new0 (gchar *, g_hash_table_size (backend->watcher_items) + 1);
      g_hash_table_foreach (backend->watcher_items,
                            sn_backend_watcher_collect_item_keys, &context);
      sn_watcher_set_registered_status_notifier_items (backend->
                                                       watcher_skeleton,
                                                       (gpointer) context.
                                                       out);
...
static gboolean
sn_backend_watcher_clear_item (gpointer key, gpointer value,
                               gpointer user_data)
{
...
{
  g_hash_table_foreach_remove (backend->watcher_items,
                               sn_backend_watcher_clear_item, NULL);
}

static gboolean
sn_backend_host_parse_name_path (const gchar * service, gchar ** bus_name,
                                 gchar ** object_path)
{
...
static void
sn_backend_host_callback (GObject * source_object, GAsyncResult * res,
                          gpointer user_data)
{
  SnBackend *backend = user_data;
  gchar *bus_name;
  ...
  if (backend->host_proxy != NULL)
    {
      g_signal_connect (backend->host_proxy,
                        "status-notifier-item-registered",
                        G_CALLBACK (sn_backend_host_item_registered),
                        backend);
      g_signal_connect (backend->host_proxy,
                        "status-notifier-item-unregistered",
                        G_CALLBACK (sn_backend_host_item_unregistered),
                        backend);
      g_signal_connect_after (backend->host_proxy, "g-properties-changed",
                              G_CALLBACK (sn_backend_host_items_changed),
                              backend);

      items =
        sn_watcher_get_registered_status_notifier_items (backend->host_proxy);
      if (items != NULL)
        {
          for (i = 0; items[i] != NULL; i++)
            {
              if (sn_backend_host_parse_name_path
                  (items[i], &bus_name, &object_path))
                {
                  sn_backend_host_add_item (backend, items[i], bus_name,
                                            object_path);
                               ...
                               gpointer user_data)
{
  ...
  sn_watcher_proxy_new (connection,
                        ...
                        backend->host_cancellable, sn_backend_host_callback,
                        backend);
}
...
static void
sn_backend_host_item_expose (SnItem * item, SnBackend * backend)
{
  g_signal_emit (G_OBJECT (backend), sn_backend_signals[ITEM_ADDED], 0, item);
}

static void
sn_backend_host_item_seal (SnItem * item, SnBackend * backend)
{
  g_signal_emit (G_OBJECT (backend), sn_backend_signals[ITEM_REMOVED], 0,
                 item);
}

static void
sn_backend_host_item_finish (SnItem * item, SnBackend * backend)
{
...
static gboolean
sn_backend_host_items_changed_remove_item (gpointer key, gpointer value,
                                           gpointer user_data)
{
...
static void
sn_backend_host_add_item (SnBackend * backend, const gchar * service,
...
static void
sn_backend_host_remove_item (SnBackend * backend, SnItem * item,
                             gboolean remove_from_table)
{
  gchar *key;
  gboolean exposed;

  g_object_get (item, "key", &key, "exposed", &exposed, NULL);
...
static gboolean
sn_backend_host_clear_item (gpointer key, gpointer value, gpointer user_data)
{
  SnBackend *backend = user_data;
  SnItem *item = value;
...
{
  g_hash_table_foreach_remove (backend->host_items,
                               sn_backend_host_clear_item, backend);
}
